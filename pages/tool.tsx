...
export default function ToolPage() {
  ...
  const btnPrimary: React.CSSProperties = {
    ...
    opacity: canUpload ? 1 : 0.6, cursor: canUpload ? "pointer" : "not-allowed",
  };
  const btnGhost: React.CSSProperties = {
    padding: "10px 16px", borderRadius: 12, border: "1px solid rgba(0,0,0,0.15)", background: "transparent",
  };
  const statusStyle: React.CSSProperties = {
    fontSize: 14, color: status==="error" ? "#b91c1c" : status==="success" ? "#065f46" : "rgba(0,0,0,0.6)"
  };
  ...
  return (
    <main style={mainStyle}>
      ...
      {/* Sticky bottom bar fallback with higher z-index */}
      <div style={barStyle} role="region" aria-label="Upload actions">
        <div style={barInner}>
